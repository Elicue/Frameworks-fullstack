<template>
  <div class=" hover:scale-[1.01] transition-all duration-500 bg-white p-3 flex flex-row justify-between w-full h-[17vh] border-t border-t-gray-200 pt-3" :class="{'shadow-[0px_0px_34px_0px_rgba(212,82,248,0.1)] duration-100' : active }">
    <div class="flex flex-row gap-5 relative w-full">
      ...
      <div class="flex flex-col justify-between w-full">
        ...
        <div class="flex flex-row gap-4">
          ...
          <Link :href="route('tracks.edit', { track: track })" class="w-fit border border-violet-500 hover:bg-violet-600 active:bg-violet-700 px-6 py-1 rounded-md text-violet-500 hover:text-white duration-500">Edit</Link>

          <Link :href="route('tracks.destroy', { track: track })" class="w-fit border border-red-500 hover:bg-red-600 active:bg-red-700 px-6 py-1 rounded-md text-red-500 hover:text-white duration-500" as="button" method="delete">Delete</Link>
        </div>
      </div>
    </div>
    <div>
