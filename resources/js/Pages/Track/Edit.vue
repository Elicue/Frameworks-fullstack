        ...
        <div class="flex flex-col w-1/2">
          <div class="flex flex-row  mb-6 justify-between w-full">
            ...
            <div class=" w-1/3 gap-4 flex flex-row justify-end">
              <Link :href="route('tracks.create')" class="w-fit bg-violet-500 hover:bg-violet-600 active:bg-violet-700 px-6 py-2 rounded-md font-bold text-white">Back</Link>
            </div>
          </div>
          <div>
            <form @submit.prevent="submit" class="w-full flex flex-col gap-4">
              ...
              <input type="submit" class="w-fit px-6 py-2 rounded-md font-bold transition-all duration-500 text-white hover:scale-[1.01]" value="Submit your modifications" :class="[form.processing ? 'bg-gray-500' : ' bg-violet-500 hover:bg-violet-600 active:bg-violet-700']" :disabled="form.processing">
